...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
        lastUnnormalizedMessages = unnormalizedMessages(lastCliqueOne, lastCliqueTwo);
        lastClique = P.cliqueList(lastCliqueTwo);
        lastPotential = FactorProduct(lastUnnormalizedMessages, lastClique);
        logZ = log(sum(lastPotential.val));
    else
        logZ = 0;
    end
