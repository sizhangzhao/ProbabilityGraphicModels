...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
numGenotype = ((numAlleles + 1) * numAlleles) / 2;
...
childAssignments = assignments(:,1);
p1Assignments = assignments(:,2);
p2Assignments = assignments(:,3);

for i = 1:length(genotypeFactor.val)
	childAlleles = genotypesToAlleles(childAssignments(i),:);
	parent1Alleles = genotypesToAlleles(p1Assignments(i), :);
	parent2Alleles = genotypesToAlleles(p2Assignments(i), :);
	for p1 = 1:2
		p1Allele = parent1Alleles(p1);
		for p2 = 1:2
			p2Allele = parent2Alleles(p2);
			if ((p1Allele == childAlleles(1) && p2Allele == childAlleles(2)) || (p1Allele == childAlleles(2) && p2Allele == childAlleles(1)))
