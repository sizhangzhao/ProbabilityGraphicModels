...
numGenotype = ((numAlleles - 1) * numAlleles) / 2 + numAlleles;
genotypeFactor.card = (numGenotype);
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
for i = 1:numGenotype
    allele = genotypesToAlleles(i,:);
    if allele(1) == allele(2)
        genotypeFactor.val(i) = alleleFreqs(allele(1)) * alleleFreqs(allele(2));
    else
        genotypeFactor.val(i) = 2 * alleleFreqs(allele(1)) * alleleFreqs(allele(2));
    end
end
