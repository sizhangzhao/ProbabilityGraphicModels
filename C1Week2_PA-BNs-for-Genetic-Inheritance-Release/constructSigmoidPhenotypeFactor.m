...
numGene = length(geneCopyVarOneList);
numAllele = zeros(1, numGene);

for i = 1:numGene
	singleNumberAllele = length(alleleWeights{i});
	numAllele(i) = singleNumberAllele;
end

phenotypeFactor.card = [2, numAllele, numAllele];
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
assignments = IndexToAssignment(1:length(phenotypeFactor.val), phenotypeFactor.card);

unnormProb = zeros(1, length(phenotypeFactor.val) / 2);

for i = 1:(length(phenotypeFactor.val) / 2)
	prob = 0;
	for j = 1:numGene
		prob = prob + alleleWeights{j}(assignments(2 * i - 1, j+1)) + alleleWeights{j}(assignments(2 * i - 1, j+numGene+1));
	end
	unnormProb(i) = prob;
end

normProb = computeSigmoid(unnormProb);
phenotypeFactor.val(1:2:end-1) = normProb;
phenotypeFactor.val(2:2:end) = 1 - normProb;

% for i = 1:length(phenotypeFactor.val)
% 	prob = 0;
% 	singleP = assignments(i, 1);
% 	for j = 1:numGene
% 		prob = prob + alleleWeights{j}(assignments(i, j+1)) + alleleWeights{j}(assignments(i, j+numGene+1));
% 	end
% 	normProb = computeSigmoid(prob);
% 	if singleP == 1
% 		phenotypeFactor.val(i) = normProb;
% 	else
% 		phenotypeFactor.val(i) = 1 - normProb;
% 	end
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%