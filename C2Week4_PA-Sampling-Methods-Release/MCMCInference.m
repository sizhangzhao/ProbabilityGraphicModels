  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              denom = 0;
              for t = 1:edge_factor.card(1)
                denom = denom + GetValueOfAssignment(edge_factor, [t t]);
              end
              q_ij_test = denom / sum(edge_factor.val);
              % index = intersect (G.var2factors{u(i)}, G.var2factors{v(i)});
              t1 = 0;
              % found_factor = F(index);
              for t = 1:edge_factor.card(1)
                  t1 = t1 + GetValueOfAssignment(edge_factor, [t t]);
              end
              q_ij = t1/sum(edge_factor.val);
              % assert(isequal(edge_factor, found_factor));
              assert(q_ij_test == q_ij);
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          else
